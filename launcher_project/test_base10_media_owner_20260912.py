...
from stability_base10_media_owner_patch_20260912 import OUT,SRC,DST
...
class SessionVM(NavVM):
 ...
 def extra(self,target,args):
  ...
  if target=='Landroid/content/Context;->getApplicationContext()Landroid/content/Context;':return Obj('application')
  raise AssertionError(target)
 def setup_media(self):
  self.current=Obj(NAV);self.manager=Obj(A);m=self.manager;m.f.update({A+'->ˋ:Landroid/content/Context;':Obj('application'),A+'->ʹ:Lo/ia;':Obj('Lo/ia;'),A+'->ˏ:Landroid/os/Handler;':Obj('handler'),A+'->ͳ:Landroid/media/session/MediaController;':Obj('controller')});self.current.f[NAV+'->ͼ:'+A]=m;self.execute('Lo/ia;','<init>('+NAV+'I)V',[m.f[A+'->ʹ:Lo/ia;'],self.current,9])
  y=Obj(Y);self.execute(Y,'<init>('+A+')V',[y,m]);m.f[A+'->ͼ:'+Y]=y;return m,y

def check(tree):
 cases=[]
 for state in ['current','manager_dead','service_dead','service_replaced','manager_replaced','callback_replaced','detached','missing_owner','wrong_owner']:
  ...
  if state=='wrong_owner':m.f[A+'->ʹ:Lo/ia;'].f['Lo/ia;->ˏ:'+NAV]=Obj('context')
 ...
 vm=SessionVM(tree);m,y=vm.setup_media();ctor=vm.life.body(A,'<init>('+NAV+')V')
 at=ctor.index('    invoke-virtual {p1}, Landroid/content/Context;->getApplicationContext()Landroid/content/Context;');last='    iput-object v0, p0, Lo/a9;->ˋ:Landroid/content/Context;';end=ctor.index(last,at)+len(last)
 vm.execute(A,'<init>('+NAV+')V',[m,vm.current],body=ctor[at:end]);assert m.f[A+'->ˋ:Landroid/content/Context;'].kind=='application'
 assert vm.execute(A,'stabilityCurrent()Z',[m])==1
 bad=SessionVM(SRC);assert bad.execute(A,'stabilityCurrent()Z',[m])==0
 cases.append('actual_constructor_application_context_reproduces_R20_rejection_fixed_owner_accepts')
 m.f[A+'->ʹ:Lo/ia;']=0;assert vm.execute(A,'stabilityCurrent()Z',[m])==0;cases.append('missing_UI_callback_rejects_owner')
...
def main():
 p=argparse.ArgumentParser();p.add_argument('--final',action='store_true');a=p.parse_args();rows=json.loads((OUT/'base10_r20b_owner_manifest.json').read_text(encoding='utf-8'))
 ...
 results=[check(DST)]
 if a.final:
  final=OUT/'final_redecode/base10_r20b'
  for rel in {r['path'] for r in rows}:assert norm(DST/rel)==norm(final/rel)
  results.append(check(final))
 (OUT/('base10_r20b_branch_tests'+('_final' if a.final else '')+'.json')).write_text(json.dumps(dict(artifacts=results,exact_parent_restoration=True),indent=2),encoding='utf-8')
 print('PASS',len(results[0]['cases']),'media owner/session groups +326 inherited')
