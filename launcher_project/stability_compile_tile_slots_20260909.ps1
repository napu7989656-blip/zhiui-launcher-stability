...
$taskOut="$taskEvidence\modules\tile_slots_r1"
...
& 'D:\Android\.jdk21\bin\javac.exe' --release 8 -Xlint:-options -encoding UTF-8 -d "$taskOut\classes" "$taskRoot\stability_modules\src\com\byd\launcher\stability\TileSlot.java" "$taskRoot\stability_modules\src\com\byd\launcher\stability\TileBatch.java"
if($LASTEXITCODE -ne 0){throw 'Production compile failed'}
& 'D:\Android\.jdk21\bin\javac.exe' --release 8 -Xlint:-options -encoding UTF-8 -cp "$taskOut\classes" -d "$taskOut\tests" "$taskRoot\stability_modules\test\TileSlotTest.java"
if($LASTEXITCODE -ne 0){throw 'Test compile failed'}
& 'D:\Android\.jdk21\bin\java.exe' -cp "$taskOut\tests;$taskOut\classes" TileSlotTest *> "$taskOut\jvm_tests.txt"
if($LASTEXITCODE -ne 0){throw 'Tile slot tests failed'}
