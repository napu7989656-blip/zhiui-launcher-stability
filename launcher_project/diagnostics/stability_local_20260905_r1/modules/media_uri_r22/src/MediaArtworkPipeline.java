...
public final class MediaArtworkPipeline {
    public interface Host extends MediaArtworkWork.Host {
        ...
        Object cached();
        Object load(String uri, MediaArtworkWork.Request request);
    ...
    private Object read(MediaArtworkWork.Request request, Input snapshot) {
        Object bitmap=null;
        if(snapshot.uri!=null && valid(request)) bitmap=host.load(snapshot.uri,request);
        if(!valid(request)) return null;
        if(bitmap!=null) return bitmap;
        if(snapshot.fallbackBitmap!=null) return snapshot.fallbackBitmap;
        if(snapshot.fallbackUri!=null && valid(request)) bitmap=host.load(snapshot.fallbackUri,request);
        return bitmap;
    }
