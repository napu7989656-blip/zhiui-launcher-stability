...
$taskOut="$taskEvidence\modules\read_after_action_r1"
...
& 'D:\Android\.jdk21\bin\javac.exe' --release 8 -Xlint:-options -encoding UTF-8 -d "$taskOut\classes" "$taskRoot\stability_modules\src\com\byd\launcher\stability\ReadAfterAction.java"
if($LASTEXITCODE -ne 0){throw 'Production compile failed'}
& 'D:\Android\.jdk21\bin\javac.exe' --release 8 -Xlint:-options -encoding UTF-8 -cp "$taskOut\classes" -d "$taskOut\tests" "$taskRoot\stability_modules\test\ReadAfterActionTest.java"
if($LASTEXITCODE -ne 0){throw 'Test compile failed'}
& 'D:\Android\.jdk21\bin\java.exe' -cp "$taskOut\tests;$taskOut\classes" ReadAfterActionTest *> "$taskOut\jvm_tests.txt"
if($LASTEXITCODE -ne 0){throw 'Read-after-action tests failed'}
